 ...
 * routines.json via Promise.all — if either of those failed, this page would never render
 * even though it never uses that data). loadExercises() resolves to the whole parsed JSON
 * object { version, meta, exercises }, not an array, so both fields are destructured from it. */
let exercises, meta;
try {
